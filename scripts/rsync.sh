...
then
    sudo rsync -azcvP \
        --log-file=/tmp/rsync-status.txt \
        --exclude=/data/transcode/ \
        --exclude=/data/letsencrypt/ \
        --exclude=/data/incomplete-downloads/ \
        --exclude=/data/libresonic/ \
        --exclude=/data/local-certs/ \
        --exclude=/data/mysql/ \
        --exclude=/data/node_modules/ \
        --exclude=/data/sab_back/ \
        --exclude=/data/transmission/ \
        /data /mnt/external
elif [ $1 = 'dry-all' ]
then
    sudo rsync -nazcvP \
        ...
        --exclude=/data/transmission/ \
        /data /mnt/external
elif [ $1 = 'sync-containers' ] 
then
    echo "Syncing podcasts..." && echo "$(date) - Syncing podcasts..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/podcasts /mnt/external/data
    echo "Syncing playlists..." && echo "$(date) - Syncing playlists..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/playlists /mnt/external/data
    echo "Syncing portainer..." && echo "$(date) - Syncing portainer..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/portainer /mnt/external/data
    echo "Syncing radarr..." && echo "$(date) - Syncing radarr..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/radarr /mnt/external/data
    echo "Syncing sonarr..." && echo "$(date) - Syncing sonarr..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/sonarr /mnt/external/data
    echo "Syncing sabnzbd..." && echo "$(date) - Syncing sabnzbd..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/sabnzbd /mnt/external/data
    echo "Syncing lidarr..." && echo "$(date) - Syncing lidarr..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/lidarr /mnt/external/data
    echo "Syncing library..." && echo "$(date) - Syncing library..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/Library /mnt/external/data
    echo "Syncing books..." && echo "$(date) - Syncing books..." >> /tmp/rsync-manual.txt
    sudo rsync -acvP --log-file=/tmp/rsync-manual.txt /data/books /mnt/external/data
elif [ $1 = 'sync-media' ]
then
